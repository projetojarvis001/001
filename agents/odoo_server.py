#!/usr/bin/env python3
import sys, os, warnings, json
warnings.filterwarnings("ignore")
sys.path.insert(0, "/Users/jarvis001/Library/Python/3.9/lib/python/site-packages")
from http.server import HTTPServer, BaseHTTPRequestHandler
from odoo_agent import process_event

class Handler(BaseHTTPRequestHandler):
    def log_message(self, fmt, *args): pass
    def do_POST(self):
        length = int(self.headers.get("Content-Length", 0))
        body = json.loads(self.rfile.read(length))
        event_type = body.get("event_type", body.get("model", "unknown"))
        data = body.get("data", body)
        try:
            result = process_event(event_type, data)
            resp = json.dumps({"ok": True, "result": result}).encode()
            self.send_response(200)
        except Exception as e:
            resp = json.dumps({"ok": False, "error": str(e)}).encode()
            self.send_response(500)
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        self.wfile.write(resp)
    def do_GET(self):
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        self.wfile.write(json.dumps({"ok": True, "service": "odoo-agent"}).encode())

if __name__ == "__main__":
    print("[OdooServer] :7780")
    HTTPServer(("0.0.0.0", 7780), Handler).serve_forever()
