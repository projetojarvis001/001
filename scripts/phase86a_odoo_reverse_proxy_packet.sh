...
TS="$(date +%Y%m%d-%H%M%S)"
OUT_JSON="logs/executive/phase86a_odoo_reverse_proxy_packet_${TS}.json"
OUT_MD="docs/generated/phase86a_odoo_reverse_proxy_packet_${TS}.md"
...
jq -nc \
  ...
  '{
    ...
    summary: {
      phase: "FASE_86A_ODOO_REVERSE_PROXY_REAL",
      flow_ok: $flow_ok,
    ...
    decision: {
      operator_note: "ODOO saiu da exposicao direta e passou a responder por Nginx real."
    },
    sources: {
...
cat > "${OUT_MD}" <<MD
# FASE 86A — ODOO Reverse Proxy Packet
...
echo "[OK] reverse proxy packet 86A gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
