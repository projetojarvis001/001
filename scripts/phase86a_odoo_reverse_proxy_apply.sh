...
TS="$(date +%Y%m%d-%H%M%S)"
RAW_FILE="runtime/odoo/reverse_proxy_apply_86a_${TS}.txt"
OUT_JSON="logs/executive/phase86a_odoo_reverse_proxy_apply_${TS}.json"
OUT_MD="docs/generated/phase86a_odoo_reverse_proxy_apply_${TS}.md"
...
ODOO_BAK='/etc/odoo.conf.bak.phase86a.${TS}'
NGINX_SITE='/etc/nginx/sites-available/odoo_phase86a'
NGINX_LINK='/etc/nginx/sites-enabled/odoo_phase86a'
...
cat > /tmp/phase86a_patch_odoo.py <<PY
...
conf.write_text('\\n'.join(new_lines) + '\\n')
print('[OK] odoo.conf phase86a atualizado')
PY

printf '%s\n' '${SSH_PASS}' | \$SUDO python3 /tmp/phase86a_patch_odoo.py
rm -f /tmp/phase86a_patch_odoo.py

cat > /tmp/odoo_phase86a.conf <<'NGINX'
server {
...
printf '%s\n' '${SSH_PASS}' | \$SUDO mv /tmp/odoo_phase86a.conf \"\${NGINX_SITE}\"
...
cat > "${OUT_MD}" <<MD
# FASE 86A — ODOO Reverse Proxy Apply
...
echo "[OK] reverse proxy apply 86A gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
