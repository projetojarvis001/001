...
OUT_DIR="logs/chaos"
OUT_FILE="${OUT_DIR}/chaos_bridge_down_${STAMP}.log"
ENV_FILE=".env"
BACKUP_FILE=".env.bak.bridge.${STAMP}"
...
p = Path(".env")
txt = p.read_text()
txt = re.sub(r'^VISION_BRIDGE_URL=.*$', 'VISION_BRIDGE_URL=http://127.0.0.1:5999/process', txt, flags=re.M)
p.write_text(txt)
print("[OK] bridge sabotado")
PY
...
echo "[OK] chaos bridge down concluido em ${DURATION}s" | tee -a "${OUT_FILE}"
