...
OUT_DIR="logs/chaos"
OUT_FILE="${OUT_DIR}/chaos_semantic_down_${STAMP}.log"
ENV_FILE=".env"
BACKUP_FILE=".env.bak.semantic.${STAMP}"
...
p = Path(".env")
txt = p.read_text()
txt = re.sub(r'^VISION_SEMANTIC_URL=.*$', 'VISION_SEMANTIC_URL=http://127.0.0.1:5998', txt, flags=re.M)
p.write_text(txt)
print("[OK] semantic sabotado")
PY
...
echo "[OK] chaos semantic down concluido em ${DURATION}s" | tee -a "${OUT_FILE}"
