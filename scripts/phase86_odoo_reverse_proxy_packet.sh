...
TS="$(date +%Y%m%d-%H%M%S)"
OUT_JSON="logs/executive/phase86_odoo_reverse_proxy_packet_${TS}.json"
OUT_MD="docs/generated/phase86_odoo_reverse_proxy_packet_${TS}.md"
...
FLOW_OK="$(jq -r '.reverse_proxy_flow.flow_ok // false' "${EVIDENCE_FILE}" 2>/dev/null || echo false)"
...
jq -nc \
  ...
  '{
    ...
    summary: {
      phase: "FASE_86_ODOO_REVERSE_PROXY_REAL",
      flow_ok: $flow_ok,
    ...
    decision: {
      operator_note: "ODOO saiu da exposicao direta e passou a responder por proxy reverso real."
    },
    sources: {
...
cat > "${OUT_MD}" <<MD
# FASE 86 — ODOO Reverse Proxy Packet
...
echo "[OK] reverse proxy packet gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
