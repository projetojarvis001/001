...
echo "===== VALIDATE FASE 86 ====="

echo
echo "===== BUILD SEED ====="
./scripts/phase86_odoo_reverse_proxy_seed.sh
SEED_FILE="$(ls -1t logs/executive/phase86_odoo_reverse_proxy_seed_*.json 2>/dev/null | head -n 1 || true)"
echo "SEED_FILE=${SEED_FILE}"

echo
echo "===== RUN APPLY ====="
./scripts/phase86_odoo_reverse_proxy_apply.sh
...
echo
echo "===== BUILD EVIDENCE ====="
./scripts/phase86_odoo_reverse_proxy_evidence.sh
EVIDENCE_FILE="$(ls -1t logs/executive/phase86_odoo_reverse_proxy_evidence_*.json 2>/dev/null | head -n 1 || true)"
echo "EVIDENCE_FILE=${EVIDENCE_FILE}"

echo
echo "===== BUILD PACKET ====="
./scripts/phase86_odoo_reverse_proxy_packet.sh
PACKET_FILE="$(ls -1t logs/executive/phase86_odoo_reverse_proxy_packet_*.json 2>/dev/null | head -n 1 || true)"
...
echo
echo "===== SANIDADE ====="
bash -n scripts/phase86_odoo_reverse_proxy_seed.sh
bash -n scripts/phase86_odoo_reverse_proxy_apply.sh
python3 -m py_compile scripts/phase86_odoo_post_proxy_probe.py
bash -n scripts/phase86_odoo_reverse_proxy_evidence.sh
bash -n scripts/phase86_odoo_reverse_proxy_packet.sh
bash -n scripts/validate_fase86.sh
echo "[OK] sintaxe shell/python valida"

echo
echo "[OK] fase 86 validada"
