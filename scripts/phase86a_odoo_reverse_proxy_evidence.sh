...
TS="$(date +%Y%m%d-%H%M%S)"
OUT_JSON="logs/executive/phase86a_odoo_reverse_proxy_evidence_${TS}.json"
OUT_MD="docs/generated/phase86a_odoo_reverse_proxy_evidence_${TS}.md"
...
if [ -z "${APPLY_FILE}" ] || [ -z "${POST_FILE}" ] || [ -z "${PACKET85}" ]; then
  echo "[ERRO] arquivos da 86A nao encontrados"
  exit 1
fi
...
HEADER_CHANGED=true
echo "${SERVER_HEADER}" | grep -qi 'Werkzeug' && HEADER_CHANGED=false || true
...
jq -nc \
  ...
  --argjson header_changed "${HEADER_CHANGED}" \
...
cat > "${OUT_MD}" <<MD
# FASE 86A — ODOO Reverse Proxy Evidence
...
echo "[OK] reverse proxy evidence 86A gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
