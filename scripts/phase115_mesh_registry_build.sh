...
OUT_STATE="registry/mesh_registry_state.json"
OUT_JSON="logs/executive/phase115_mesh_registry_build_${TS}.json"
OUT_MD="docs/generated/phase115_mesh_registry_build_${TS}.md"

APPLY_OK="$(jq -r '.mesh_registry_apply.overall_ok' "${APPLY_FILE}")"
PROBE_OK="$(jq -r '.mesh_registry_probe.overall_ok' "${PROBE_FILE}")"
READY_COUNT="$(jq -r '.mesh_registry_probe.ready_count' "${PROBE_FILE}")"

STATUS="partial"
if [ "${APPLY_OK}" = "true" ] && [ "${PROBE_OK}" = "true" ] && [ "${READY_COUNT}" = "3" ]; then
  STATUS="registry_operational"
fi
...
echo "[OK] phase115 build gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
echo
