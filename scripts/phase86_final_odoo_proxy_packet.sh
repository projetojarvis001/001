...
TS="$(date +%Y%m%d-%H%M%S)"
OUT_JSON="logs/executive/phase86_final_odoo_proxy_packet_${TS}.json"
OUT_MD="docs/generated/phase86_final_odoo_proxy_packet_${TS}.md"
...
FLOW_OK="$(jq -r '.proxy_final_flow.flow_ok // false' "${EVIDENCE_FILE}" 2>/dev/null || echo false)"
...
print(f"{min(before + 1.2, 10.0):.1f}")
PY
)"
fi
...
jq -nc \
  ...
  '{
    ...
    summary: {
      phase: "FASE_86_FINAL_ODOO_PROXY_REAL",
      flow_ok: $flow_ok,
    ...
    decision: {
      operator_note: "ODOO passou a responder por Nginx real na borda, com Odoo interno protegido."
    },
    sources: {
...
cat > "${OUT_MD}" <<MD
# FASE 86 FINAL — ODOO PROXY PACKET
...
echo "[OK] phase86 final packet gerado em ${OUT_JSON}"
cat "${OUT_JSON}" | jq .
